...
export class UbigeoService {
  ...
  getProvincesById(idDepartment: string): any[] {
    let provincesObtained = [];
    this.provinces.forEach(e => {
      if (e.department_id == idDepartment) {
        provincesObtained.push(e);
      }
    });
    return provincesObtained;
  }
  getDistrictsById(idProvince: string): any[] {
    let districtObtained = [];
    this.districts.forEach(e => {
      if (e.province_id == idProvince) {
        districtObtained.push(e);
      }
    });
    return districtObtained;
  }

  getUbigeo(idDepartment, idProvince, idDistrict) {
    let department;
    let province;
    let district;
